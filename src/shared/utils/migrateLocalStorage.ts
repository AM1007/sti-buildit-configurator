...
export function migrateLocalStorage(): void {
  try {
    ...
    if (targetRaw) {
      ...
      if (target?.state && isNonEmptyArray(target.state.guestConfigurations)) {
        cleanupLegacyKey()
        return
      }
    }
    ...
    const migratedMeta = isPlainObject(projectMeta)
      ? {
          projectName:
            typeof projectMeta.projectName === 'string' ? projectMeta.projectName : '',
          clientName:
            typeof projectMeta.clientName === 'string' ? projectMeta.clientName : '',
          createdAt:
            typeof projectMeta.createdAt === 'number' ? projectMeta.createdAt : now,
          updatedAt:
            typeof projectMeta.updatedAt === 'number' ? projectMeta.updatedAt : now,
          date:
            typeof projectMeta.date === 'string' ? projectMeta.date : defaultMeta.date,
          lastExportedAt:
