...
import { downloadMyListXlsx } from '../lib/generateMyListXlsx'
import { buildMyListShareUrl } from '@shared/utils/configSerializer'
...
export function StickyExportBar() {
  ...
  const totalUnits = myList.reduce(
  ...
  const handleExport = async (meta: {
    ...
    try {
      ...
      if (isAuthenticated && isProjectDetail) {
        await updateProjectMeta(activeProjectId, {
          lastExportedAt: new Date(exportedAt).toISOString(),
        })
      } else {
