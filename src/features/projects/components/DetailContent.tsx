    ...
    <div className="px-5 py-4 border-b border-slate-200 flex items-start justify-between bg-slate-50">
      <div>
        <div className="flex items-center gap-2 mb-1">
          ...
          <span
            className={`px-1.5 py-0.5 rounded-sm text-[10px] md:text-[9px] font-semibold tracking-wider uppercase border ${
              isCustom
        ...
        <h2 className="text-base md:text-sm font-medium text-slate-900">{modelName}</h2>
      </div>
      <button
        ref={closeButtonRef}
        type="button"
...
export function DetailBody({ item }: DetailBodyProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-6">
      ...
      {item.customText?.submitted && <CustomTextDisplay customText={item.customText} />}
  ...
  return (
    <div className="p-5 border-t border-slate-200 bg-slate-50 flex flex-col gap-2">
      <button
        ...
        className="w-full py-2.5 md:py-2 bg-slate-900 text-white text-sm md:text-xs font-medium rounded-sm hover:bg-slate-800 transition-colors flex items-center justify-center gap-1.5"
      >
        {isCustom ? t('myList.openInConfigurator') : t('myList.viewProductPage')}
      </button>
      <button
        type="button"
        onClick={handleRemove}
        className="w-full py-2.5 md:py-2 text-red-600 border border-red-200 text-sm md:text-xs font-medium rounded-sm hover:bg-red-50 transition-colors flex items-center justify-center gap-1.5"
      >
...
function ConfigurationSummary({ item }: { item: SavedConfiguration }) {
  ...
  const entries = useMemo(() => {
    ...
    return model.stepOrder
      .filter(
        (stepId) =>
          item.configuration[stepId] !== null && item.configuration[stepId] !== undefined,
      )
      .map((stepId) => ({
        stepId,
  ...
  return (
    <div>
      <h3 className="text-[11px] md:text-[10px] font-semibold uppercase tracking-wider text-slate-500 mb-3">
      ...
      <div className="border border-slate-200 rounded-sm divide-y divide-slate-100 bg-white">
        {entries.length === 0 && !isLoaded ? (
          <div className="py-3 px-3 text-sm md:text-xs text-slate-400">
            {t('common.loading')}
          </div>
        ) : (
          entries.map((entry, index) => (
            <div
              key={entry.stepId}
              className={`py-2.5 md:py-2 px-3 ${index % 2 === 1 ? 'bg-slate-50' : ''}`}
            >
              <span className="text-[11px] md:text-[10px] uppercase tracking-wider text-slate-400 font-semibold block mb-0.5">
                {entry.label}
              </span>
              <span className="text-sm md:text-xs font-medium text-slate-900 block text-left">
                {entry.value}
              </span>
            </div>
          ))
    ...
    <div>
      ...
      <div className="border border-slate-200 rounded-sm divide-y divide-slate-100 bg-white">
        {!isLoaded ? (
          ...
          <div className="py-2.5 md:py-2 px-3">
            <span className="text-[11px] md:text-[10px] uppercase tracking-wider text-slate-400 font-semibold block mb-0.5">
              Category
            </span>
            <span className="text-sm md:text-xs font-medium text-slate-900 block text-left">
              {meta?.series ??
                MODEL_NAMES[modelId as keyof typeof MODEL_NAMES] ??
                modelId}
            </span>
          </div>
        )}
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        <div>
          <span className="text-[11px] md:text-[10px] uppercase tracking-wider text-slate-400 font-semibold block mb-1">
            {t('drawer.quantityAssigned')}
          </span>
          <span className="text-sm md:text-xs font-mono font-medium text-slate-900 bg-slate-50 border border-slate-200 px-2 py-1 rounded-sm inline-block">
            {qty}
          </span>
        </div>
        <div>
          ...
          <p className="text-sm md:text-xs text-slate-700 bg-slate-50 border border-slate-200 px-2.5 py-2 rounded-sm leading-relaxed">
...
function CopySku({ sku }: { sku: string }) {
  ...
  return (
    ...
    >
      <span className="text-sm md:text-xs font-mono text-slate-900">{sku}</span>
      {copied ? (
