...
const descriptionModules = import.meta.glob<{ default: ModelDescriptions }>(
  '../../../shared/i18n/locales/*/modelDescriptions/*.json',
  { eager: false },
)
...
function buildGlobPath(lang: Language, modelId: string): string {
  return `../../../shared/i18n/locales/${lang}/modelDescriptions/${modelId}.json`
}
...
async function loadModelDescriptions(
  ...
  if (!loader) {
    console.warn(`[getModelDescription] No description file found for path: ${globPath}`)
    console.warn(
...
export async function getModelDescription(
  ...
  if (!descriptions && lang !== 'en') {
    console.warn(`[getModelDescription] Falling back to EN for ${modelId}`)
    descriptions = await loadModelDescriptions(modelId, 'en')
  }

  if (!descriptions) {
    console.warn(`[getModelDescription] No descriptions available for model: ${modelId}`)
    return null
  }
