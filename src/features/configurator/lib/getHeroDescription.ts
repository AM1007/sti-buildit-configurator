...
const translationModules = import.meta.glob<{ default: ModelTranslations }>(
  '../../../shared/i18n/locales/*/models/*.json',
  { eager: false },
)
...
function buildGlobPath(lang: Language, modelId: string): string {
  return `../../../shared/i18n/locales/${lang}/models/${modelId}.json`
}
...
async function loadModelTranslations(
  ...
  if (!loader) {
    console.warn(`[getHeroDescription] No translation file found for path: ${globPath}`)
    return null
  }
...
export async function getHeroDescription(
  ...
  if (!translations && lang !== 'en') {
    console.warn(`[getHeroDescription] Falling back to EN for ${modelId}`)
    translations = await loadModelTranslations(modelId, 'en')
  }

  if (!translations?.meta?.heroDescription) {
    console.warn(`[getHeroDescription] No heroDescription found for model: ${modelId}`)
    return null
  }
