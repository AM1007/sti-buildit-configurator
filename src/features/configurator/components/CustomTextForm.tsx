...
import type { CustomTextData, CustomTextVariant } from '@shared/types'
...
export function CustomTextForm({
  ...
  const [selectedLineCount, setSelectedLineCount] = useState<1 | 2 | 3>(
    getInitialLineCount(),
  )
  ...
  const [coverLineCount, setCoverLineCount] = useState<1 | 2 | 3>(
    getInitialCoverLineCount(),
  )
  ...
  const showLineCountSelector =
    variant === 'multiline-selectable' || variant === 'dual-block-three-line'
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (line1.length > maxLength) {
      validationErrors.push(
        t('customText.lineExceedsMax', { line: '1', max: maxLength.toString() }),
      )
    }
    ...
    if (showLine3 && line3.length > maxLength) {
      validationErrors.push(
        t('customText.lineExceedsMax', { line: '3', max: maxLength.toString() }),
      )
    }
    ...
    if (isDualBlock) {
      ...
      if (coverLine1.length > maxLength) {
        validationErrors.push(
          t('customText.coverLineExceedsMax', { line: '1', max: maxLength.toString() }),
        )
      }

      if (coverLineCount >= 2 && coverLine2.length > maxLength) {
        validationErrors.push(
          t('customText.coverLineExceedsMax', { line: '2', max: maxLength.toString() }),
        )
      }

      if (coverLineCount >= 3 && coverLine3.length > maxLength) {
        validationErrors.push(
          t('customText.coverLineExceedsMax', { line: '3', max: maxLength.toString() }),
        )
      }
    }
