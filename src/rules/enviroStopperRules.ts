import type { ModelConstraints, ConstraintMatrix } from "./types";

const ALL_COLOUR_LABELS = ["FR", "NR", "CR", "EG", "NG", "CG", "NK", "CK", "NB", "CB", "NC", "NW", "CW", "NY", "CY"];
const ALL_COLOUR_LABELS_WITHOUT_NC = ALL_COLOUR_LABELS.filter(c => c !== "NC");
const ALL_COLOUR_LABELS_WITHOUT_CK = ALL_COLOUR_LABELS.filter(c => c !== "CK");

const COVER_TO_COLOUR_LABEL: ConstraintMatrix = {
  "13": ALL_COLOUR_LABELS,
  "14": ALL_COLOUR_LABELS_WITHOUT_CK,
};

const HOOD_SOUNDER_TO_COLOUR_LABEL: ConstraintMatrix = {
  "00": ["NC"],
  "10": ALL_COLOUR_LABELS_WITHOUT_NC,
  "20": ALL_COLOUR_LABELS_WITHOUT_NC,
  "30": ALL_COLOUR_LABELS_WITHOUT_NC,
};

const COLOUR_LABEL_TO_HOOD_SOUNDER: ConstraintMatrix = {
  "FR": ["10", "20", "30"],
  "NR": ["10", "20", "30"],
  "CR": ["10", "20", "30"],
  "EG": ["10", "20", "30"],
  "NG": ["10", "20", "30"],
  "CG": ["10", "20", "30"],
  "NK": ["10", "20", "30"],
  "CK": ["10", "20", "30"],
  "NB": ["10", "20", "30"],
  "CB": ["10", "20", "30"],
  "NC": ["00"],
  "NW": ["10", "20", "30"],
  "CW": ["10", "20", "30"],
  "NY": ["10", "20", "30"],
  "CY": ["10", "20", "30"],
};

const COLOUR_LABEL_TO_COVER: ConstraintMatrix = {
  "FR": ["13", "14"],
  "NR": ["13", "14"],
  "CR": ["13", "14"],
  "EG": ["13", "14"],
  "NG": ["13", "14"],
  "CG": ["13", "14"],
  "NK": ["13", "14"],
  "CK": ["13"],
  "NB": ["13", "14"],
  "CB": ["13", "14"],
  "NC": ["13", "14"],
  "NW": ["13", "14"],
  "CW": ["13", "14"],
  "NY": ["13", "14"],
  "CY": ["13", "14"],
};

export const ENVIRO_STOPPER_CONSTRAINTS: ModelConstraints = {
  modelId: "enviro-stopper",
  constraints: [
    {
      sourceStep: "cover",
      targetStep: "colourLabel",
      matrix: COVER_TO_COLOUR_LABEL,
    },
    {
      sourceStep: "hoodSounder",
      targetStep: "colourLabel",
      matrix: HOOD_SOUNDER_TO_COLOUR_LABEL,
    },
    {
      sourceStep: "colourLabel",
      targetStep: "hoodSounder",
      matrix: COLOUR_LABEL_TO_HOOD_SOUNDER,
    },
    {
      sourceStep: "colourLabel",
      targetStep: "cover",
      matrix: COLOUR_LABEL_TO_COVER,
    },
  ],
};