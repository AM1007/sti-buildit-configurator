...
export function CustomTextDisplay({ customText }: CustomTextDisplayProps) {
  ...
  return (
    <div className="mt-2 border-2 border-black bg-white p-4 md:p-5">
      <h4 className="mb-2 text-lg font-bold text-black md:text-xl xl:text-2xl">{t("customTextDisplay.title")}</h4>
      <p className="text-base font-normal text-brand-600 md:text-lg">
        {t("customTextDisplay.nonReturnable")}
      </p>

      <div className="ml-4 mt-3 md:ml-6 md:mt-4">
        <ul className="grid gap-4">
          <li>
            <div>
              <div className="relative mb-1">
                <span className="absolute left-0 top-1/2 inline-block h-2 w-2 -translate-y-1/2 rounded-full bg-brand-600" />
                <span className="ml-4 text-sm font-bold text-black md:text-base">{t("customText.label")}</span>
              </div>
          ...
          {hasCoverData && (
            <li>
              <div>
                <div className="relative mb-1">
                  <span className="absolute left-0 top-1/2 inline-block h-2 w-2 -translate-y-1/2 rounded-full bg-brand-600" />
                  <span className="ml-4 text-sm font-bold text-black md:text-base">{t("customText.cover")}</span>
                </div>
                <div className="grid grid-cols-1 gap-1">
