...
import { waterproofPushButtonsModel } from '@entities/product/models/waterproofPushButtons'
...
describe('buildWPBModelCode', () => {
  ...
  it('returns null when any field is missing', () => {
    expect(
      buildWPBModelCode({ housingColour: '1', buttonColour: 'R', buttonType: '0' }),
    ).toBeNull()
    expect(
      buildWPBModelCode({ housingColour: '1', buttonColour: 'R', label: 'SAK' }),
    ).toBeNull()
...
describe('VALID_MODEL_CODES', () => {
  ...
  it('orange buttonColour (E) only with orange housing (E)', () => {
    const ebCodes = VALID_MODEL_CODES.filter(
      (c) => parseWPBModelCode(c)?.buttonColour === 'E',
    )
...
describe('isValidWPBCombination', () => {
  ...
  it('returns valid for incomplete selection', () => {
    ...
    expect(
      isValidWPBCombination({ housingColour: '1', buttonColour: 'R', buttonType: '0' }),
    ).toEqual({ valid: true })
  })
...
describe('WATERPROOF_PUSH_BUTTONS_CONSTRAINTS + constraintEngine', () => {
  ...
  it('blocks buttonType=1 when housingColour=E', () => {
    expect(
      engine.checkOptionAvailability('buttonType', '1', { housingColour: 'E' }).available,
    ).toBe(false)
  })

  it('allows buttonType=0 when housingColour=E', () => {
    expect(
      engine.checkOptionAvailability('buttonType', '0', { housingColour: 'E' }).available,
    ).toBe(true)
  })

  it('blocks CL label when housingColour=E', () => {
    expect(
      engine.checkOptionAvailability('label', 'CL', { housingColour: 'E' }).available,
    ).toBe(false)
  })

  it('allows SAK label when housingColour=E', () => {
    expect(
      engine.checkOptionAvailability('label', 'SAK', { housingColour: 'E' }).available,
    ).toBe(true)
  })
  ...
  it('blocks buttonColour=E when housingColour is not E', () => {
    for (const housing of ['1', '3', '5', '7', '9']) {
      expect(
        engine.checkOptionAvailability('buttonColour', 'E', { housingColour: housing })
          .available,
      ).toBe(false)
    }
  })
  ...
  it('blocks buttonColour=Y when housingColour is not 5', () => {
    for (const housing of ['1', '3', '7', '9', 'E']) {
      expect(
        engine.checkOptionAvailability('buttonColour', 'Y', { housingColour: housing })
          .available,
      ).toBe(false)
    }
  })
  ...
  it('blocks buttonColour=G when housingColour=1 or 5 or 9', () => {
    for (const housing of ['1', '5', '9']) {
      expect(
        engine.checkOptionAvailability('buttonColour', 'G', { housingColour: housing })
          .available,
      ).toBe(false)
    }
  })
  ...
  it('blocks buttonColour=E when buttonType=1', () => {
    expect(
      engine.checkOptionAvailability('buttonColour', 'E', { buttonType: '1' }).available,
    ).toBe(false)
  })
...
describe('isConfigurationComplete — waterproofPushButtons', () => {
  ...
  it('returns false when any step missing', () => {
    expect(
      isConfigurationComplete(waterproofPushButtonsModel, {
        housingColour: '1',
        buttonColour: 'R',
        buttonType: '0',
        electricalArrangements: '4',
...
describe('waterproofPushButtonsModel definition', () => {
  ...
  it('electricalArrangements step has exactly one option — 4', () => {
    const eaStep = waterproofPushButtonsModel.steps.find(
      (s) => s.id === 'electricalArrangements',
    )!
