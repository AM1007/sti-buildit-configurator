...
import { gfFireAlarmPushButtonModel } from "../data/models/gfFireAlarmPushButton";
...
describe("buildGFModelCode", () => {
  it("builds all 6 valid SKUs correctly", () => {
    expect(buildGFModelCode({ model: "A", cover: "0", text: "FR", language: "EN" }))
      .toBe("GFA0FR-EN");
    expect(buildGFModelCode({ model: "A", cover: "0", text: "HF", language: "EN" }))
      .toBe("GFA0HF-EN");
    expect(buildGFModelCode({ model: "A", cover: "2", text: "FR", language: "EN" }))
      .toBe("GFA2FR-EN");
    expect(buildGFModelCode({ model: "C", cover: "0", text: "FR", language: "EN" }))
      .toBe("GFC0FR-EN");
    expect(buildGFModelCode({ model: "C", cover: "0", text: "HF", language: "EN" }))
      .toBe("GFC0HF-EN");
    expect(buildGFModelCode({ model: "C", cover: "2", text: "FR", language: "EN" }))
      .toBe("GFC2FR-EN");
  });
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("cover 2 only appears with FR text", () => {
    const cover2Codes = VALID_MODEL_CODES.filter((c) => parseGFModelCode(c)?.cover === "2");
...
describe("isValidGFCombination", () => {
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidGFCombination({ model: "A", cover: "0", text: "FR" })).toEqual({ valid: true });
  });

  it("rejects HF text with cover 2 — not in allowlist", () => {
    const result = isValidGFCombination({ model: "A", cover: "2", text: "HF", language: "EN" });
    expect(result.valid).toBe(false);
    if (!result.valid) expect(result.reason).toContain("GFA2HF-EN");
  });

  it("rejects HF text with cover 2 for model C as well", () => {
    const result = isValidGFCombination({ model: "C", cover: "2", text: "HF", language: "EN" });
    expect(result.valid).toBe(false);
  });
});
...
describe("GF_FIRE_ALARM_PUSH_BUTTON_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks HF text when cover is 2", () => {
    expect(engine.checkOptionAvailability("text", "HF", { cover: "2" }).available)
      .toBe(false);
  });

  it("allows FR text when cover is 2", () => {
    expect(engine.checkOptionAvailability("text", "FR", { cover: "2" }).available)
      .toBe(true);
  });

  it("blocks cover 2 when text is HF", () => {
    expect(engine.checkOptionAvailability("cover", "2", { text: "HF" }).available)
      .toBe(false);
  });

  it("allows cover 0 when text is HF", () => {
    expect(engine.checkOptionAvailability("cover", "0", { text: "HF" }).available)
      .toBe(true);
  });
  ...
  it("language always EN — no restriction from any step", () => {
    expect(engine.checkOptionAvailability("language", "EN", { model: "A" }).available)
      .toBe(true);
    expect(engine.checkOptionAvailability("language", "EN", { cover: "2" }).available)
      .toBe(true);
    expect(engine.checkOptionAvailability("language", "EN", { text: "HF" }).available)
      .toBe(true);
  });

  it("constraint engine modelId matches", () => {
    expect(GF_FIRE_ALARM_PUSH_BUTTON_CONSTRAINTS.modelId).toBe("gf-fire-alarm-push-button");
  });
});
...
describe("isConfigurationComplete — gfFireAlarmPushButton", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(gfFireAlarmPushButtonModel, {
      model: "A", cover: "0", text: "FR", language: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage for 4-step model", () => {
    expect(getCompletionPercentage(gfFireAlarmPushButtonModel, {
      model: null, cover: null, text: null, language: null,
    })).toBe(0);

    expect(getCompletionPercentage(gfFireAlarmPushButtonModel, {
      model: "A", cover: null, text: null, language: null,
    })).toBe(25);

    expect(getCompletionPercentage(gfFireAlarmPushButtonModel, {
      model: "A", cover: "0", text: null, language: null,
    })).toBe(50);
    ...
    expect(getCompletionPercentage(gfFireAlarmPushButtonModel, {
      model: "A", cover: "0", text: "FR", language: "EN",
    })).toBe(100);
  });
});
