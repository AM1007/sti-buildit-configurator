...
import { globalResetModel } from "../data/models/globalReset";
...
describe("buildGLRModelCode", () => {
  it("builds GLR001ZA-EN correctly", () => {
    expect(buildGLRModelCode({ colour: "0", cover: "01", text: "ZA", language: "EN" }))
      .toBe("GLR001ZA-EN");
  });

  it("builds GLR101RM-EN correctly", () => {
    expect(buildGLRModelCode({ colour: "1", cover: "01", text: "RM", language: "EN" }))
      .toBe("GLR101RM-EN");
  });

  it("builds GLR401EV-EN correctly", () => {
    expect(buildGLRModelCode({ colour: "4", cover: "01", text: "EV", language: "EN" }))
      .toBe("GLR401EV-EN");
  });

  it("cover 01 is two characters — GLR{1}{2}{2}-{2}", () => {
    const code = buildGLRModelCode({ colour: "2", cover: "01", text: "ZA", language: "EN" });
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("colour distribution: 0→1, 1→3, 2→4, 3→1, 4→4", () => {
    for (const [colour, count] of [["0", 1], ["1", 3], ["2", 4], ["3", 1], ["4", 4]] as const) {
      const filtered = VALID_MODEL_CODES.filter((c) => parseGLRModelCode(c)?.colour === colour);
  ...
  it("NT and PS texts only valid with colour 2", () => {
    for (const text of ["NT", "PS"]) {
      const textCodes = VALID_MODEL_CODES.filter((c) => parseGLRModelCode(c)?.text === text);
  ...
  it("colour 0 and 3 only support ZA text", () => {
    for (const colour of ["0", "3"]) {
      const colourCodes = VALID_MODEL_CODES.filter((c) => parseGLRModelCode(c)?.colour === colour);
...
describe("isValidGLRCombination", () => {
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidGLRCombination({ colour: "0", cover: "01", text: "ZA" }))
      .toEqual({ valid: true });
  });

  it("rejects ZA text with colour 1 — not in allowlist", () => {
    const result = isValidGLRCombination({ colour: "1", cover: "01", text: "ZA", language: "EN" });
    expect(result.valid).toBe(false);
    if (!result.valid) expect(result.reason).toContain("GLR101ZA-EN");
  });

  it("rejects RM text with colour 0", () => {
    const result = isValidGLRCombination({ colour: "0", cover: "01", text: "RM", language: "EN" });
    expect(result.valid).toBe(false);
  });

  it("rejects EV text with any colour except 4", () => {
    for (const colour of ["0", "1", "2", "3"]) {
      const result = isValidGLRCombination({ colour, cover: "01", text: "EV", language: "EN" });
      expect(result.valid).toBe(false);
    }
  });
  ...
  it("rejects NT text with any colour except 2", () => {
    for (const colour of ["0", "1", "3", "4"]) {
      const result = isValidGLRCombination({ colour, cover: "01", text: "NT", language: "EN" });
      expect(result.valid).toBe(false);
    }
  });
});
...
describe("getValidGLROptionsForStep", () => {
  ...
  it("language always returns only EN", () => {
    ...
    expect(getValidGLROptionsForStep("language", { colour: "2", text: "ZA" })).toEqual(["EN"]);
  });
...
describe("GLOBAL_RESET_CONSTRAINTS + constraintEngine", () => {
  ...
  it("only colour↔text constraints — cover and language unconstrained", () => {
    ...
    const steps = GLOBAL_RESET_CONSTRAINTS.constraints.map((c) => [c.sourceStep, c.targetStep]);
    expect(steps).toContainEqual(["colour", "text"]);
    expect(steps).toContainEqual(["text", "colour"]);
  });

  it("blocks ZA text when colour is 1", () => {
    expect(engine.checkOptionAvailability("text", "ZA", { colour: "1" }).available)
      .toBe(false);
  });

  it("allows ZA text when colour is 0, 2, 3, 4", () => {
    for (const colour of ["0", "2", "3", "4"]) {
      expect(engine.checkOptionAvailability("text", "ZA", { colour }).available)
        .toBe(true);
    }
  });
  ...
  it("blocks RM text when colour is not 1", () => {
    for (const colour of ["0", "2", "3", "4"]) {
      expect(engine.checkOptionAvailability("text", "RM", { colour }).available)
        .toBe(false);
    }
  });

  it("allows RM text when colour is 1", () => {
    expect(engine.checkOptionAvailability("text", "RM", { colour: "1" }).available)
      .toBe(true);
  });
  ...
  it("blocks colour 1 when text is EV, LD, NT, or PS", () => {
    for (const text of ["EV", "LD", "NT", "PS"]) {
      expect(engine.checkOptionAvailability("colour", "1", { text }).available)
        .toBe(false);
    }
  });
  ...
  it("blocks colour 0 when text is EM, EX, RM, NT, PS, EV, or LD", () => {
    for (const text of ["EM", "EX", "RM", "NT", "PS", "EV", "LD"]) {
      expect(engine.checkOptionAvailability("colour", "0", { text }).available)
        .toBe(false);
    }
  });
...
describe("buildProductModel — globalReset", () => {
  ...
  it("baseCode is GLR", () => {
    const config: Configuration = { colour: null, cover: null, text: null, language: null };
    const result = buildProductModel(config, globalResetModel);
...
describe("isConfigurationComplete — globalReset", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(globalResetModel, {
      colour: "0", cover: "01", text: "ZA", language: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage for 4-step model", () => {
    expect(getCompletionPercentage(globalResetModel, {
      colour: null, cover: null, text: null, language: null,
    })).toBe(0);

    expect(getCompletionPercentage(globalResetModel, {
      colour: "0", cover: null, text: null, language: null,
    })).toBe(25);

    expect(getCompletionPercentage(globalResetModel, {
      colour: "0", cover: "01", text: null, language: null,
    })).toBe(50);
    ...
    expect(getCompletionPercentage(globalResetModel, {
      colour: "0", cover: "01", text: "ZA", language: "EN",
    })).toBe(100);
  });
});
