...
import { universalStopperModel } from "../data/models/universalStopper";
...
describe("buildUSModelCode", () => {
  it("builds correct code from full selection", () => {
    expect(buildUSModelCode({ mounting: "0", hoodSounder: "10", colourLabel: "FR" }))
      .toBe("STI-13010FR");
  });

  it("builds correct code for surface mount with sounder", () => {
    expect(buildUSModelCode({ mounting: "1", hoodSounder: "20", colourLabel: "EG" }))
      .toBe("STI-13120EG");
  });
...
describe("isValidUSCombination", () => {
  it("returns valid for known good combinations", () => {
    expect(isValidUSCombination({ mounting: "0", hoodSounder: "10", colourLabel: "FR" }))
      .toEqual({ valid: true });
    expect(isValidUSCombination({ mounting: "1", hoodSounder: "20", colourLabel: "EG" }))
      .toEqual({ valid: true });
    expect(isValidUSCombination({ mounting: "2", hoodSounder: "30", colourLabel: "FR" }))
      .toEqual({ valid: true });
  });

  it("returns valid for incomplete selection (user still picking)", () => {
    expect(isValidUSCombination({ mounting: "0" })).toEqual({ valid: true });
    expect(isValidUSCombination({ mounting: "0", hoodSounder: "10" })).toEqual({ valid: true });
    expect(isValidUSCombination({})).toEqual({ valid: true });
  });

  it("returns invalid for non-existent combination", () => {
    const result = isValidUSCombination({ mounting: "2", hoodSounder: "00", colourLabel: "NC" });
...
describe("UNIVERSAL_STOPPER_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks NC colourLabel when hoodSounder=10", () => {
    const result = engine.checkOptionAvailability("colourLabel", "NC", { hoodSounder: "10" });
    expect(result.available).toBe(false);
  });

  it("allows NC colourLabel when hoodSounder=00", () => {
    const result = engine.checkOptionAvailability("colourLabel", "NC", { hoodSounder: "00" });
    expect(result.available).toBe(true);
  });
...
describe("buildProductModel — universalStopper", () => {
  ...
  it("generated SKUs for all valid combinations exist in VALID_MODEL_CODES", () => {
    ...
    const colourLabels = ["FR", "NR", "EG", "NG", "NC", "CK", "NB", "CB", "NW", "CW", "NY", "CY", "CR", "CG"];
...
describe("isConfigurationComplete — universalStopper", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(universalStopperModel, {
      mounting: "0", hoodSounder: "10", colourLabel: null,
    })).toBe(false);

    expect(isConfigurationComplete(universalStopperModel, {
      mounting: null, hoodSounder: null, colourLabel: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage returns correct percentages", () => {
    expect(getCompletionPercentage(universalStopperModel, {
      mounting: null, hoodSounder: null, colourLabel: null,
    })).toBe(0);

    expect(getCompletionPercentage(universalStopperModel, {
      mounting: "0", hoodSounder: null, colourLabel: null,
    })).toBe(33);
    ...
    expect(getCompletionPercentage(universalStopperModel, {
      mounting: "0", hoodSounder: "10", colourLabel: "FR",
    })).toBe(100);
  });
});
...
describe("universalStopperModel definition", () => {
  ...
  it("partsOrder matches stepOrder", () => {
    expect(universalStopperModel.productModelSchema.partsOrder)
      .toEqual(universalStopperModel.stepOrder);
  });

  it("all option codes match SKU segment format", () => {
    const mountingCodes = universalStopperModel.steps
      .find((s) => s.id === "mounting")!.options.map((o) => o.code);
    expect(mountingCodes).toEqual(["0", "1", "2"]);

    const hoodCodes = universalStopperModel.steps
      .find((s) => s.id === "hoodSounder")!.options.map((o) => o.code);
