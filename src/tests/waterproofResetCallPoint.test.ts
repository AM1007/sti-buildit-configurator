...
import { waterproofResetCallPointModel } from "../data/models/waterproofResetCallPoint";
...
describe("buildWRPModelCode", () => {
  it("builds WRP2-R-02 correctly — HF no suffix", () => {
    expect(buildWRPModelCode({ colour: "R", electricalArrangement: "02", label: "HF" }))
      .toBe("WRP2-R-02");
  });

  it("builds WRP2-R-11-CL correctly — CL suffix", () => {
    expect(buildWRPModelCode({ colour: "R", electricalArrangement: "11", label: "CL" }))
      .toBe("WRP2-R-11-CL");
  });

  it("builds WRP2-G-02 correctly — RM no suffix", () => {
    expect(buildWRPModelCode({ colour: "G", electricalArrangement: "02", label: "RM" }))
      .toBe("WRP2-G-02");
  });

  it("builds WRP2-Y-11-CL correctly — SAK colour with CL", () => {
    expect(buildWRPModelCode({ colour: "Y", electricalArrangement: "11", label: "CL" }))
      .toBe("WRP2-Y-11-CL");
  });

  it("builds WRP2-O-02 correctly — SAK no suffix", () => {
    expect(buildWRPModelCode({ colour: "O", electricalArrangement: "02", label: "SAK" }))
      .toBe("WRP2-O-02");
  });

  it("separator structure: WRP2-{colour}-{ea}", () => {
    const code = buildWRPModelCode({ colour: "B", electricalArrangement: "11", label: "SAK" });
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("ea=02: 11 codes, ea=11: 12 codes", () => {
    expect(VALID_MODEL_CODES.filter((c) => parseWRPModelCode(c)?.electricalArrangement === "02").length).toBe(11);
    expect(VALID_MODEL_CODES.filter((c) => parseWRPModelCode(c)?.electricalArrangement === "11").length).toBe(12);
  });
  ...
  it("non-red/green colours have symmetric 4-code pattern: 02, 02-CL, 11, 11-CL", () => {
    for (const colour of ["Y", "W", "B", "O"]) {
      const colourCodes = VALID_MODEL_CODES.filter((c) => parseWRPModelCode(c)?.colour === colour);
...
describe("isValidWRPCombination", () => {
  it("all 23 VALID_MODEL_CODES pass validation", () => {
    for (const code of VALID_MODEL_CODES) {
      const parsed = parseWRPModelCode(code)!;
      const colour = parsed.colour!;
      const label = parsed.label ?? (colour === "R" ? "HF" : colour === "G" ? "RM" : "SAK");
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidWRPCombination({ colour: "R", electricalArrangement: "02" })).toEqual({ valid: true });
  });

  it("rejects false positive WRP2-R-02-CL", () => {
    const result = isValidWRPCombination({ colour: "R", electricalArrangement: "02", label: "CL" });
  ...
  it("rejects HF label with non-red colour", () => {
    for (const colour of ["G", "Y", "W", "B", "O"]) {
      const result = isValidWRPCombination({ colour, electricalArrangement: "02", label: "HF" });
      expect(result.valid).toBe(false);
    }
  });
  ...
  it("rejects RM label with non-green colour", () => {
    for (const colour of ["R", "Y", "W", "B", "O"]) {
      const result = isValidWRPCombination({ colour, electricalArrangement: "02", label: "RM" });
      expect(result.valid).toBe(false);
    }
  });
  ...
  it("rejects SAK label with red or green colour", () => {
    for (const colour of ["R", "G"]) {
      const result = isValidWRPCombination({ colour, electricalArrangement: "02", label: "SAK" });
      expect(result.valid).toBe(false);
    }
  });

  it("rejects ea=01 — absent from allowlist", () => {
    const result = isValidWRPCombination({ colour: "G", electricalArrangement: "01", label: "RM" });
    expect(result.valid).toBe(false);
  });
});
...
describe("WATERPROOF_RESET_CALL_POINT_CONSTRAINTS + constraintEngine", () => {
  ...
  it("allows RM label when colour is G", () => {
    expect(engine.checkOptionAvailability("label", "RM", { colour: "G" }).available)
      .toBe(true);
  });

  it("blocks SAK label when colour is R or G", () => {
    for (const colour of ["R", "G"]) {
      expect(engine.checkOptionAvailability("label", "SAK", { colour }).available)
        .toBe(false);
    }
  });
  ...
  it("CL label is available for all colours via constraint matrix", () => {
    for (const colour of ["R", "G", "Y", "W", "B", "O"]) {
      expect(engine.checkOptionAvailability("label", "CL", { colour }).available)
        .toBe(true);
    }
  });
  ...
  it("colour and ea are fully independent via matrices", () => {
    for (const colour of ["R", "G", "Y", "W", "B", "O"]) {
      for (const ea of ["02", "11"]) {
        expect(engine.checkOptionAvailability("electricalArrangement", ea, { colour }).available)
          .toBe(true);
      }
    }
  });
  ...
  it("constraint engine modelId matches", () => {
    expect(WATERPROOF_RESET_CALL_POINT_CONSTRAINTS.modelId).toBe("waterproof-reset-call-point");
  });
});
...
describe("buildProductModel — waterproofResetCallPoint", () => {
  it("builds WRP2-R-02 correctly — HF label", () => {
    const config: Configuration = { colour: "R", electricalArrangement: "02", label: "HF" };
  ...
  it("builds WRP2-R-11-CL correctly", () => {
    const config: Configuration = { colour: "R", electricalArrangement: "11", label: "CL" };
    const result = buildProductModel(config, waterproofResetCallPointModel);
  ...
  it("builds WRP2-G-02 correctly — RM label no suffix", () => {
    const config: Configuration = { colour: "G", electricalArrangement: "02", label: "RM" };
    const result = buildProductModel(config, waterproofResetCallPointModel);
  ...
  it("builds WRP2-O-11-CL correctly", () => {
    const config: Configuration = { colour: "O", electricalArrangement: "11", label: "CL" };
    const result = buildProductModel(config, waterproofResetCallPointModel);
  ...
  it("all separators are dashes — WRP2-{colour}-{ea}", () => {
    const config: Configuration = { colour: "B", electricalArrangement: "11", label: "SAK" };
    const result = buildProductModel(config, waterproofResetCallPointModel);
  ...
  it("baseCode is WRP2", () => {
    const config: Configuration = { colour: null, electricalArrangement: null, label: null };
    const result = buildProductModel(config, waterproofResetCallPointModel);
    expect(result.baseCode).toBe("WRP2");
  });

  it("marks incomplete when steps missing", () => {
    const config: Configuration = { colour: "G", electricalArrangement: null, label: null };
    const result = buildProductModel(config, waterproofResetCallPointModel);
...
describe("isConfigurationComplete — waterproofResetCallPoint", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(waterproofResetCallPointModel, {
      colour: "R", electricalArrangement: "02", label: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage for 3-step model", () => {
    expect(getCompletionPercentage(waterproofResetCallPointModel, {
      colour: null, electricalArrangement: null, label: null,
    })).toBe(0);

    expect(getCompletionPercentage(waterproofResetCallPointModel, {
      colour: "R", electricalArrangement: null, label: null,
    })).toBe(33);
    ...
    expect(getCompletionPercentage(waterproofResetCallPointModel, {
      colour: "R", electricalArrangement: "02", label: "HF",
    })).toBe(100);
  });
});
...
describe("waterproofResetCallPointModel definition", () => {
  ...
  it("electricalArrangement step has 2 options — 02 and 11, no 01", () => {
    const eaStep = waterproofResetCallPointModel.steps.find((s) => s.id === "electricalArrangement")!;
