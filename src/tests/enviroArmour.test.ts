...
import { enviroArmourModel } from '@entities/product/models/enviroArmour'
...
describe('buildEAModelCode', () => {
  it('builds all 6 valid SKUs correctly', () => {
    expect(buildEAModelCode({ material: 'S', size: '121005', doorType: 'O' })).toBe(
      'ES-121005-O',
    )
    expect(buildEAModelCode({ material: 'S', size: '161608', doorType: 'O' })).toBe(
      'ES-161608-O',
    )
    expect(buildEAModelCode({ material: 'S', size: '231609', doorType: 'O' })).toBe(
      'ES-231609-O',
    )
    expect(buildEAModelCode({ material: 'S', size: '312312', doorType: 'O' })).toBe(
      'ES-312312-O',
    )
    expect(buildEAModelCode({ material: 'T', size: '121006', doorType: 'C' })).toBe(
      'ET-121006-C',
    )
    expect(buildEAModelCode({ material: 'T', size: '181408', doorType: 'C' })).toBe(
      'ET-181408-C',
    )
  })
...
describe('isValidEACombination', () => {
  ...
  it('returns valid for incomplete selection', () => {
    ...
    expect(isValidEACombination({ material: 'S', size: '121005' })).toEqual({
      valid: true,
    })
  })
...
describe('isConfigurationComplete — enviroArmour', () => {
  ...
  it('returns false when any step missing', () => {
    expect(
      isConfigurationComplete(enviroArmourModel, {
        material: 'S',
        size: '121005',
        doorType: null,
      }),
  ...
  it('getCompletionPercentage returns correct percentages', () => {
    expect(
      getCompletionPercentage(enviroArmourModel, {
        material: null,
        size: null,
        doorType: null,
      }),
