...
import { callPointStopperModel } from "../data/models/callPointStopper";
...
describe("buildCPSModelCode", () => {
  it("builds flush red fire alarm — no suffixes", () => {
    expect(buildCPSModelCode({ mounting: "0", colour: "R", label: "FIRE" }))
      .toBe("STI-6930");
  });

  it("builds surface red fire alarm — no colour suffix", () => {
    expect(buildCPSModelCode({ mounting: "1", colour: "R", label: "FIRE" }))
      .toBe("STI-6931");
  });

  it("builds surface red custom label", () => {
    expect(buildCPSModelCode({ mounting: "1", colour: "R", label: "CL" }))
      .toBe("STI-6931-CL");
  });

  it("builds flush green plain", () => {
    expect(buildCPSModelCode({ mounting: "0", colour: "G", label: "PLAIN" }))
      .toBe("STI-6930-G-PLAIN");
  });

  it("builds surface yellow custom label", () => {
    expect(buildCPSModelCode({ mounting: "1", colour: "Y", label: "CL" }))
      .toBe("STI-6931-Y-CL");
  });

  it("builds surface blue plain", () => {
    expect(buildCPSModelCode({ mounting: "1", colour: "B", label: "PLAIN" }))
      .toBe("STI-6931-B-PLAIN");
  });

  it("builds surface orange emergency operate — no label suffix", () => {
    expect(buildCPSModelCode({ mounting: "1", colour: "E", label: "EMERGENCY_OPERATE" }))
      .toBe("STI-6931-E");
  });
...
describe("getValidCPSOptionsForStep", () => {
  ...
  it("surface mount enables more label options for blue than flush", () => {
    const validSurface = getValidCPSOptionsForStep("label", { mounting: "1", colour: "B" });
  ...
  it("white colour has no CL option in either mounting", () => {
    ...
    const validSurface = getValidCPSOptionsForStep("label", { mounting: "1", colour: "W" });
    expect(validFlush).not.toContain("CL");
...
describe("CALL_POINT_STOPPER_CONSTRAINTS + constraintEngine", () => {
  ...
  it("EMERGENCY_DOOR label blocks red colour", () => {
    const result = engine.checkOptionAvailability("colour", "R", { label: "EMERGENCY_DOOR" });
    expect(result.available).toBe(false);
  });
  ...
  it("green colour blocks FIRE and CL labels", () => {
    ...
    expect(
      engine.checkOptionAvailability("label", "CL", { colour: "G" }).available
    ).toBe(false);
  });

  it("green colour allows EMERGENCY_DOOR and PLAIN labels", () => {
    expect(
      engine.checkOptionAvailability("label", "EMERGENCY_DOOR", { colour: "G" }).available
    ).toBe(true);
    expect(
...
describe("isConfigurationComplete — callPointStopper", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(callPointStopperModel, {
      mounting: "0", colour: "R", label: null,
    })).toBe(false);

    expect(isConfigurationComplete(callPointStopperModel, {
      mounting: null, colour: null, label: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage returns correct percentages", () => {
    expect(getCompletionPercentage(callPointStopperModel, {
      mounting: null, colour: null, label: null,
    })).toBe(0);

    expect(getCompletionPercentage(callPointStopperModel, {
      mounting: "0", colour: null, label: null,
    })).toBe(33);
    ...
    expect(getCompletionPercentage(callPointStopperModel, {
      mounting: "0", colour: "R", label: "FIRE",
    })).toBe(100);
  });
});
