...
import { indoorPushButtonsModel } from "../data/models/indoorPushButtons";
...
describe("buildIPBModelCode", () => {
  it("builds SAK label code — no suffix", () => {
    expect(buildIPBModelCode({
      colour: "1", buttonColour: "R", pushButtonType: "1",
  ...
  it("builds pneumatic code correctly", () => {
    expect(buildIPBModelCode({
      colour: "3", buttonColour: "G", pushButtonType: "6",
      electricalArrangements: "0", label: "SAK",
    })).toBe("SS3-3G60");
  });

  it("builds orange housing orange button correctly", () => {
    expect(buildIPBModelCode({
      colour: "E", buttonColour: "E", pushButtonType: "0",
      electricalArrangements: "4", label: "SAK",
    })).toBe("SS3-EE04");
  });
  ...
  it("returns null when any field is missing", () => {
    expect(buildIPBModelCode({
      colour: "1", buttonColour: "R", pushButtonType: "1", electricalArrangements: "4",
    })).toBeNull();
    expect(buildIPBModelCode({
      colour: "1", buttonColour: "R", pushButtonType: "1", label: "SAK",
    })).toBeNull();
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("pushButtonType distribution: 0→24, 1→26, 6→9", () => {
    ...
    expect(VALID_MODEL_CODES.filter((c) => parse(c)?.pushButtonType === "0").length).toBe(24);
    expect(VALID_MODEL_CODES.filter((c) => parse(c)?.pushButtonType === "1").length).toBe(26);
    expect(VALID_MODEL_CODES.filter((c) => parse(c)?.pushButtonType === "6").length).toBe(9);
  });
  ...
  it("pneumatic (pbt=6) always has electricalArrangements=0", () => {
    const pbt6 = VALID_MODEL_CODES.filter((c) => parseIPBModelCode(c)?.pushButtonType === "6");
  ...
  it("key-to-reset (pbt=0) and momentary (pbt=1) always have electricalArrangements=4", () => {
    for (const pbt of ["0", "1"]) {
      const codes = VALID_MODEL_CODES.filter((c) => parseIPBModelCode(c)?.pushButtonType === pbt);
  ...
  it("colour 1 (red housing) and E (orange) never have pbt=6", () => {
    for (const colour of ["1", "E"]) {
      const codes = VALID_MODEL_CODES.filter((c) => parseIPBModelCode(c)?.colour === colour);
      for (const code of codes) {
...
describe("isValidIPBCombination", () => {
  ...
  it("rejects pbt=6 with colour=1 — not in allowlist", () => {
    const result = isValidIPBCombination({
      colour: "1", buttonColour: "R", pushButtonType: "6",
      electricalArrangements: "0", label: "SAK",
    });
  ...
  it("rejects pbt=6 with colour=E — not in allowlist", () => {
    const result = isValidIPBCombination({
      colour: "E", buttonColour: "E", pushButtonType: "6",
      electricalArrangements: "0", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects pbt=1 with electricalArrangements=0 — pbt=1 only with ea=4", () => {
    const result = isValidIPBCombination({
      colour: "3", buttonColour: "G", pushButtonType: "1",
      electricalArrangements: "0", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects colour=1 with buttonColour=G — not in allowlist", () => {
    const result = isValidIPBCombination({
      colour: "1", buttonColour: "G", pushButtonType: "1",
      electricalArrangements: "4", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
...
describe("getValidIPBOptionsForStep", () => {
  ...
  it("pbt=6 (pneumatic) forces electricalArrangements=0", () => {
    const valid = getValidIPBOptionsForStep("electricalArrangements", { pushButtonType: "6" });
    expect(valid).toEqual(["0"]);
  });

  it("pbt=0 and pbt=1 force electricalArrangements=4", () => {
    for (const pbt of ["0", "1"]) {
      const valid = getValidIPBOptionsForStep("electricalArrangements", { pushButtonType: pbt });
      expect(valid).toEqual(["4"]);
    }
  });

  it("electricalArrangements=0 only allows pbt=6", () => {
    const valid = getValidIPBOptionsForStep("pushButtonType", { electricalArrangements: "0" });
  ...
  it("buttonColour=B and E not available with pbt=6 or ea=0", () => {
    ...
    const ea0Valid = getValidIPBOptionsForStep("buttonColour", { electricalArrangements: "0" });
...
describe("INDOOR_PUSH_BUTTONS_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks pbt=6 when colour=1", () => {
    expect(engine.checkOptionAvailability("pushButtonType", "6", { colour: "1" }).available)
      .toBe(false);
  });

  it("blocks pbt=6 when colour=E", () => {
    expect(engine.checkOptionAvailability("pushButtonType", "6", { colour: "E" }).available)
      .toBe(false);
  });

  it("allows pbt=6 when colour=3, 5, 7, or 9", () => {
    for (const colour of ["3", "5", "7", "9"]) {
      expect(engine.checkOptionAvailability("pushButtonType", "6", { colour }).available)
        .toBe(true);
    }
  });
  ...
  it("blocks ea=0 when pbt=0 or pbt=1", () => {
    for (const pbt of ["0", "1"]) {
      expect(engine.checkOptionAvailability("electricalArrangements", "0", { pushButtonType: pbt }).available)
        .toBe(false);
    }
  });

  it("blocks ea=4 when pbt=6", () => {
    expect(engine.checkOptionAvailability("electricalArrangements", "4", { pushButtonType: "6" }).available)
      .toBe(false);
  });

  it("blocks ea=0 when colour=1 or E", () => {
    for (const colour of ["1", "E"]) {
      expect(engine.checkOptionAvailability("electricalArrangements", "0", { colour }).available)
        .toBe(false);
    }
  });

  it("blocks buttonColour=G when colour=1", () => {
    expect(engine.checkOptionAvailability("buttonColour", "G", { colour: "1" }).available)
      .toBe(false);
  });

  it("blocks buttonColour=E when colour is not E", () => {
    for (const colour of ["1", "3", "5", "7", "9"]) {
      expect(engine.checkOptionAvailability("buttonColour", "E", { colour }).available)
        .toBe(false);
    }
  });
...
describe("buildProductModel — indoorPushButtons", () => {
  ...
  it("builds SS3-9R60-CL correctly", () => {
    const config: Configuration = {
      colour: "9", buttonColour: "R", pushButtonType: "6",
      electricalArrangements: "0", label: "CL",
    };
    const result = buildProductModel(config, indoorPushButtonsModel);
  ...
  it("SAK label has empty code — no suffix, no extra dash", () => {
    const config: Configuration = {
      colour: "7", buttonColour: "W", pushButtonType: "0",
      electricalArrangements: "4", label: "SAK",
    };
    const result = buildProductModel(config, indoorPushButtonsModel);
  ...
  it("baseCode is SS3-", () => {
    const config: Configuration = {
      colour: null, buttonColour: null, pushButtonType: null,
  ...
  it("marks incomplete when steps missing", () => {
    const config: Configuration = {
      colour: "1", buttonColour: "R", pushButtonType: null,
      electricalArrangements: null, label: null,
    };
    const result = buildProductModel(config, indoorPushButtonsModel);
...
describe("isConfigurationComplete — indoorPushButtons", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(indoorPushButtonsModel, {
      colour: "1", buttonColour: "R", pushButtonType: "1",
  ...
  it("getMissingRequiredSteps returns correct missing steps", () => {
    const config: Configuration = {
      colour: "3", buttonColour: "G", pushButtonType: null,
      electricalArrangements: null, label: null,
    };
  ...
  it("getCompletionPercentage for 5-step model", () => {
    expect(getCompletionPercentage(indoorPushButtonsModel, {
      colour: null, buttonColour: null, pushButtonType: null,
      electricalArrangements: null, label: null,
    })).toBe(0);

    expect(getCompletionPercentage(indoorPushButtonsModel, {
      colour: "1", buttonColour: null, pushButtonType: null,
      electricalArrangements: null, label: null,
    })).toBe(20);
