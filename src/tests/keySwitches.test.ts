...
import { keySwitchesModel } from "../data/models/keySwitches";
...
describe("buildKSModelCode", () => {
  it("builds SAK label — no suffix", () => {
    expect(buildKSModelCode({ colourMounting: "10", switchType: "2", electricalArrangement: "0", label: "SAK" }))
      .toBe("SS3-1020");
  });

  it("builds CL label — appends -CL", () => {
    expect(buildKSModelCode({ colourMounting: "10", switchType: "2", electricalArrangement: "0", label: "CL" }))
      .toBe("SS3-1020-CL");
  });

  it("builds orange housing correctly — E0 colourMounting", () => {
    expect(buildKSModelCode({ colourMounting: "E0", switchType: "5", electricalArrangement: "3", label: "SAK" }))
      .toBe("SS3-E053");
    expect(buildKSModelCode({ colourMounting: "E0", switchType: "5", electricalArrangement: "3", label: "CL" }))
      .toBe("SS3-E053-CL");
  });

  it("builds SS3-1053 correctly", () => {
    expect(buildKSModelCode({ colourMounting: "10", switchType: "5", electricalArrangement: "3", label: "SAK" }))
      .toBe("SS3-1053");
  });

  it("returns null when any field is missing", () => {
    expect(buildKSModelCode({ colourMounting: "10", switchType: "2", electricalArrangement: "0" })).toBeNull();
    expect(buildKSModelCode({ colourMounting: "10", switchType: "2", label: "SAK" })).toBeNull();
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("E0 (orange) never has switchType=4", () => {
    const e0 = VALID_MODEL_CODES.filter((c) => parseKSModelCode(c)?.colourMounting === "E0");
  ...
  it("colourMounting=30 and 90 never have electricalArrangement=3", () => {
    for (const cm of ["30", "90"]) {
      const codes = VALID_MODEL_CODES.filter((c) => parseKSModelCode(c)?.colourMounting === cm);
...
describe("isValidKSCombination", () => {
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidKSCombination({ colourMounting: "10", switchType: "2", electricalArrangement: "0" }))
      .toEqual({ valid: true });
  });

  it("rejects E0 with switchType=4 — not in allowlist", () => {
    const result = isValidKSCombination({
      colourMounting: "E0", switchType: "4", electricalArrangement: "1", label: "SAK",
    });
  ...
  it("rejects switchType=2 with electricalArrangement=1 — st=2 only ea=0", () => {
    const result = isValidKSCombination({
      colourMounting: "10", switchType: "2", electricalArrangement: "1", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects switchType=5 with electricalArrangement=0 — st=5 only ea=3", () => {
    const result = isValidKSCombination({
      colourMounting: "10", switchType: "5", electricalArrangement: "0", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects colourMounting=30 with electricalArrangement=3", () => {
    const result = isValidKSCombination({
      colourMounting: "30", switchType: "5", electricalArrangement: "3", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects colourMounting=90 with electricalArrangement=3", () => {
    const result = isValidKSCombination({
      colourMounting: "90", switchType: "5", electricalArrangement: "3", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
});
...
describe("getValidKSOptionsForStep", () => {
  ...
  it("colourMounting=30 and 90 do not allow electricalArrangement=3", () => {
    for (const cm of ["30", "90"]) {
      const valid = getValidKSOptionsForStep("electricalArrangement", { colourMounting: cm });
...
describe("KEY_SWITCHES_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks switchType=4 when colourMounting=E0", () => {
    expect(engine.checkOptionAvailability("switchType", "4", { colourMounting: "E0" }).available)
      .toBe(false);
  });

  it("allows switchType=2, 3, 5 when colourMounting=E0", () => {
    for (const st of ["2", "3", "5"]) {
      expect(engine.checkOptionAvailability("switchType", st, { colourMounting: "E0" }).available)
        .toBe(true);
    }
  });
  ...
  it("blocks switchType=4 when colourMounting=30 or 90", () => {
    for (const cm of ["30", "90"]) {
      expect(engine.checkOptionAvailability("switchType", "4", { colourMounting: cm }).available)
        .toBe(false);
    }
  });
  ...
  it("blocks electricalArrangement=1 and 2 when switchType=2", () => {
    for (const ea of ["1", "2"]) {
      expect(engine.checkOptionAvailability("electricalArrangement", ea, { switchType: "2" }).available)
        .toBe(false);
    }
  });

  it("allows electricalArrangement=0 when switchType=2", () => {
    expect(engine.checkOptionAvailability("electricalArrangement", "0", { switchType: "2" }).available)
      .toBe(true);
  });

  it("blocks electricalArrangement=0 when switchType=5", () => {
    expect(engine.checkOptionAvailability("electricalArrangement", "0", { switchType: "5" }).available)
      .toBe(false);
  });

  it("allows electricalArrangement=3 only when switchType=5", () => {
    expect(engine.checkOptionAvailability("electricalArrangement", "3", { switchType: "5" }).available)
      .toBe(true);
    for (const st of ["2", "3", "4"]) {
      expect(engine.checkOptionAvailability("electricalArrangement", "3", { switchType: st }).available)
        .toBe(false);
    }
  });
  ...
  it("blocks electricalArrangement=3 when colourMounting=30 or 90", () => {
    for (const cm of ["30", "90"]) {
      expect(engine.checkOptionAvailability("electricalArrangement", "3", { colourMounting: cm }).available)
        .toBe(false);
    }
  });
  ...
  it("label not included in constraint matrices — 6 constraints total", () => {
    ...
    const steps = KEY_SWITCHES_CONSTRAINTS.constraints.flatMap((c) => [c.sourceStep, c.targetStep]);
...
describe("buildProductModel — keySwitches", () => {
  it("builds SS3-1020 correctly", () => {
    const config: Configuration = {
      colourMounting: "10", switchType: "2", electricalArrangement: "0", label: "SAK",
    };
  ...
  it("builds SS3-1020-CL correctly", () => {
    const config: Configuration = {
      colourMounting: "10", switchType: "2", electricalArrangement: "0", label: "CL",
    };
    const result = buildProductModel(config, keySwitchesModel);
  ...
  it("builds SS3-E053-CL correctly", () => {
    const config: Configuration = {
      colourMounting: "E0", switchType: "5", electricalArrangement: "3", label: "CL",
    };
    const result = buildProductModel(config, keySwitchesModel);
  ...
  it("SAK label has empty code — no suffix", () => {
    const config: Configuration = {
      colourMounting: "30", switchType: "3", electricalArrangement: "0", label: "SAK",
    };
    const result = buildProductModel(config, keySwitchesModel);
  ...
  it("baseCode is SS3-", () => {
    const config: Configuration = {
      colourMounting: null, switchType: null, electricalArrangement: null, label: null,
    };
    const result = buildProductModel(config, keySwitchesModel);
  ...
  it("marks incomplete when steps missing", () => {
    const config: Configuration = {
      colourMounting: "10", switchType: null, electricalArrangement: null, label: null,
    };
    const result = buildProductModel(config, keySwitchesModel);
...
describe("isConfigurationComplete — keySwitches", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(keySwitchesModel, {
      colourMounting: "10", switchType: "2", electricalArrangement: "0", label: null,
    })).toBe(false);
  });

  it("getMissingRequiredSteps returns correct missing steps", () => {
    const config: Configuration = {
      colourMounting: "E0", switchType: null, electricalArrangement: null, label: null,
    };
  ...
  it("getCompletionPercentage for 4-step model", () => {
    expect(getCompletionPercentage(keySwitchesModel, {
      colourMounting: null, switchType: null, electricalArrangement: null, label: null,
    })).toBe(0);
