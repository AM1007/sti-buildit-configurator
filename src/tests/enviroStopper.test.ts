...
import { enviroStopperModel } from "../data/models/enviroStopper";
...
describe("buildESModelCode", () => {
  it("builds dome sealed no-hood clear — STI-13600NC", () => {
    expect(buildESModelCode({ cover: "13", mounting: "6", hoodSounder: "00", colourLabel: "NC" }))
      .toBe("STI-13600NC");
  });

  it("builds dome sealed label hood fire red — STI-13610FR", () => {
    expect(buildESModelCode({ cover: "13", mounting: "6", hoodSounder: "10", colourLabel: "FR" }))
      .toBe("STI-13610FR");
  });

  it("builds low-profile open sounder relay green — STI-14730EG", () => {
    expect(buildESModelCode({ cover: "14", mounting: "7", hoodSounder: "30", colourLabel: "EG" }))
      .toBe("STI-14730EG");
  });

  it("builds dome open label hood red no label — STI-13710NR", () => {
    expect(buildESModelCode({ cover: "13", mounting: "7", hoodSounder: "10", colourLabel: "NR" }))
      .toBe("STI-13710NR");
  });
  ...
  it("returns null when any field is missing", () => {
    ...
    expect(buildESModelCode({ cover: "13", hoodSounder: "00", colourLabel: "NC" })).toBeNull();
    expect(buildESModelCode({ mounting: "6", hoodSounder: "00", colourLabel: "NC" })).toBeNull();
...
describe("ENVIRO_STOPPER_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks CR and CB colourLabels when cover=14", () => {
    for (const colour of ["CR", "CB"]) {
      const result = engine.checkOptionAvailability("colourLabel", colour, { cover: "14" });
      expect(result.available).toBe(false);
    }
  });

  it("hoodSounder=30 only allows FR and EG via constraint matrix", () => {
    const allColours = ["FR", "EG", "CR", "CG", "CY", "CW", "CB", "NC", "NR", "NG", "NY", "NW", "NB"];
    for (const colour of allColours) {
      const result = engine.checkOptionAvailability("colourLabel", colour, { hoodSounder: "30" });
...
describe("isConfigurationComplete — enviroStopper", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(enviroStopperModel, {
      cover: "13", mounting: "6", hoodSounder: "00", colourLabel: null,
    })).toBe(false);
  ...
  it("getCompletionPercentage returns correct percentages for 4-step model", () => {
    expect(getCompletionPercentage(enviroStopperModel, {
      cover: null, mounting: null, hoodSounder: null, colourLabel: null,
    })).toBe(0);

    expect(getCompletionPercentage(enviroStopperModel, {
      cover: "13", mounting: null, hoodSounder: null, colourLabel: null,
    })).toBe(25);

    expect(getCompletionPercentage(enviroStopperModel, {
      cover: "13", mounting: "6", hoodSounder: null, colourLabel: null,
    })).toBe(50);

    expect(getCompletionPercentage(enviroStopperModel, {
      cover: "13", mounting: "6", hoodSounder: "10", colourLabel: null,
    })).toBe(75);

    expect(getCompletionPercentage(enviroStopperModel, {
      cover: "13", mounting: "6", hoodSounder: "00", colourLabel: "NC",
    })).toBe(100);
  });
});
