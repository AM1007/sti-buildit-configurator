...
import { resetCallPointsModel } from "../data/models/resetCallPoints";
...
describe("buildRPModelCode", () => {
  it("builds red HF — no suffix", () => {
    expect(buildRPModelCode({ colour: "R", mounting: "D2", electricalArrangement: "02", label: "HF" }))
      .toBe("RP-RD2-02");
  });

  it("builds green RM — no suffix", () => {
    expect(buildRPModelCode({ colour: "G", mounting: "D2", electricalArrangement: "11", label: "RM" }))
      .toBe("RP-GD2-11");
  });

  it("builds yellow SAK — no suffix", () => {
    expect(buildRPModelCode({ colour: "Y", mounting: "F2", electricalArrangement: "02", label: "SAK" }))
      .toBe("RP-YF2-02");
  });

  it("builds green CL — appends -CL", () => {
    expect(buildRPModelCode({ colour: "G", mounting: "D2", electricalArrangement: "02", label: "CL" }))
      .toBe("RP-GD2-02-CL");
  });

  it("builds orange CL correctly", () => {
    expect(buildRPModelCode({ colour: "O", mounting: "F2", electricalArrangement: "02", label: "CL" }))
      .toBe("RP-OF2-02-CL");
  });

  it("separator structure: RP-{colour}{mounting}-{ea}", () => {
    const code = buildRPModelCode({ colour: "B", mounting: "S2", electricalArrangement: "11", label: "SAK" });
  ...
  it("returns null when any field is missing", () => {
    expect(buildRPModelCode({ colour: "R", mounting: "D2", electricalArrangement: "02" })).toBeNull();
...
describe("VALID_MODEL_CODES", () => {
  ...
  it("ea=02: 26 codes, ea=11: 24 codes", () => {
    expect(VALID_MODEL_CODES.filter((c) => parseRPModelCode(c)?.electricalArrangement === "02").length).toBe(26);
    expect(VALID_MODEL_CODES.filter((c) => parseRPModelCode(c)?.electricalArrangement === "11").length).toBe(24);
  });

  it("ea=01 and ea=05 absent — blocked globally", () => {
    expect(VALID_MODEL_CODES.some((c) => parseRPModelCode(c)?.electricalArrangement === "01")).toBe(false);
    expect(VALID_MODEL_CODES.some((c) => parseRPModelCode(c)?.electricalArrangement === "05")).toBe(false);
  });
...
describe("isValidRPCombination", () => {
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidRPCombination({ colour: "R", mounting: "D2", electricalArrangement: "02" }))
      .toEqual({ valid: true });
  });

  it("rejects red with F2 mounting — not in allowlist", () => {
    const result = isValidRPCombination({
      colour: "R", mounting: "F2", electricalArrangement: "02", label: "HF",
    });
  ...
  it("rejects red with CL label", () => {
    const result = isValidRPCombination({
      colour: "R", mounting: "D2", electricalArrangement: "02", label: "CL",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects ea=01 — globally blocked", () => {
    const result = isValidRPCombination({
      colour: "G", mounting: "D2", electricalArrangement: "01", label: "RM",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects ea=05 — globally blocked", () => {
    const result = isValidRPCombination({
      colour: "Y", mounting: "S2", electricalArrangement: "05", label: "SAK",
    });
    expect(result.valid).toBe(false);
  });
  ...
  it("rejects HF label with non-red colour", () => {
    for (const colour of ["G", "Y", "W", "B", "O"]) {
      const result = isValidRPCombination({
        colour, mounting: "D2", electricalArrangement: "02", label: "HF",
      });
      expect(result.valid).toBe(false);
    }
  });
  ...
  it("rejects RM label with non-green colour", () => {
    for (const colour of ["R", "Y", "W", "B", "O"]) {
      const result = isValidRPCombination({
        colour, mounting: "D2", electricalArrangement: "02", label: "RM",
      });
      expect(result.valid).toBe(false);
    }
  });
...
describe("RESET_CALL_POINTS_CONSTRAINTS + constraintEngine", () => {
  ...
  it("blocks F2 mounting when colour is R", () => {
    expect(engine.checkOptionAvailability("mounting", "F2", { colour: "R" }).available)
      .toBe(false);
  });

  it("allows D2 and S2 mounting when colour is R", () => {
    expect(engine.checkOptionAvailability("mounting", "D2", { colour: "R" }).available)
      .toBe(true);
    expect(engine.checkOptionAvailability("mounting", "S2", { colour: "R" }).available)
      .toBe(true);
  });

  it("blocks R colour when mounting is F2", () => {
    expect(engine.checkOptionAvailability("colour", "R", { mounting: "F2" }).available)
      .toBe(false);
  });

  it("blocks HF label when colour is not R", () => {
    for (const colour of ["G", "Y", "W", "B", "O"]) {
      expect(engine.checkOptionAvailability("label", "HF", { colour }).available)
        .toBe(false);
    }
  });

  it("allows HF label when colour is R", () => {
    expect(engine.checkOptionAvailability("label", "HF", { colour: "R" }).available)
      .toBe(true);
  });

  it("blocks RM label when colour is not G", () => {
    for (const colour of ["R", "Y", "W", "B", "O"]) {
      expect(engine.checkOptionAvailability("label", "RM", { colour }).available)
        .toBe(false);
    }
  });

  it("blocks SAK label when colour is R or G", () => {
    expect(engine.checkOptionAvailability("label", "SAK", { colour: "R" }).available)
      .toBe(false);
    expect(engine.checkOptionAvailability("label", "SAK", { colour: "G" }).available)
      .toBe(false);
  });

  it("blocks HF label when mounting is F2", () => {
    expect(engine.checkOptionAvailability("label", "HF", { mounting: "F2" }).available)
      .toBe(false);
  });
...
describe("buildProductModel — resetCallPoints", () => {
  it("builds RP-RD2-02 correctly — red HF no suffix", () => {
    const config: Configuration = {
      colour: "R", mounting: "D2", electricalArrangement: "02", label: "HF",
    };
  ...
  it("builds RP-GD2-02-CL correctly — green CL", () => {
    const config: Configuration = {
      colour: "G", mounting: "D2", electricalArrangement: "02", label: "CL",
    };
    const result = buildProductModel(config, resetCallPointsModel);
  ...
  it("builds RP-YF2-11-CL correctly", () => {
    const config: Configuration = {
      colour: "Y", mounting: "F2", electricalArrangement: "11", label: "CL",
    };
    const result = buildProductModel(config, resetCallPointsModel);
  ...
  it("HF, RM, SAK labels have empty code — no suffix", () => {
    for (const [colour, label] of [["R", "HF"], ["G", "RM"], ["Y", "SAK"]] as const) {
      const config: Configuration = { colour, mounting: "D2", electricalArrangement: "02", label };
      const result = buildProductModel(config, resetCallPointsModel);
  ...
  it("baseCode is RP", () => {
    const config: Configuration = {
      colour: null, mounting: null, electricalArrangement: null, label: null,
    };
    const result = buildProductModel(config, resetCallPointsModel);
  ...
  it("separator structure: RP-{colour}{mounting}-{ea}", () => {
    const config: Configuration = {
      colour: "B", mounting: "S2", electricalArrangement: "11", label: "SAK",
    };
    const result = buildProductModel(config, resetCallPointsModel);
  ...
  it("marks incomplete when steps missing", () => {
    const config: Configuration = {
      colour: "G", mounting: null, electricalArrangement: null, label: null,
    };
    const result = buildProductModel(config, resetCallPointsModel);
...
describe("isConfigurationComplete — resetCallPoints", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(resetCallPointsModel, {
      colour: "R", mounting: "D2", electricalArrangement: "02", label: null,
    })).toBe(false);
  });

  it("getMissingRequiredSteps returns correct missing steps", () => {
    const config: Configuration = {
      colour: "G", mounting: "D2", electricalArrangement: null, label: null,
    };
  ...
  it("getCompletionPercentage for 4-step model", () => {
    expect(getCompletionPercentage(resetCallPointsModel, {
      colour: null, mounting: null, electricalArrangement: null, label: null,
    })).toBe(0);

    expect(getCompletionPercentage(resetCallPointsModel, {
      colour: "R", mounting: null, electricalArrangement: null, label: null,
    })).toBe(25);
...
describe("resetCallPointsModel definition", () => {
  ...
  it("electricalArrangement step has 4 options including blocked 01 and 05", () => {
    const eaStep = resetCallPointsModel.steps.find((s) => s.id === "electricalArrangement")!;
