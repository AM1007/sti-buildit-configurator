...
import { euroStopperModel } from "../data/models/euroStopper";
...
describe("buildEUSModelCode", () => {
  it("builds flush no-sounder blue custom — STI-15010CB", () => {
    expect(buildEUSModelCode({ mounting: "0", sounder: "10", colourLabel: "CB" }))
      .toBe("STI-15010CB");
  });

  it("builds surface 32mm sounder relay green — STI-15C30NG", () => {
    expect(buildEUSModelCode({ mounting: "C", sounder: "30", colourLabel: "NG" }))
      .toBe("STI-15C30NG");
  });

  it("builds surface 50mm sounder relay emergency green — STI-15D30EG", () => {
    expect(buildEUSModelCode({ mounting: "D", sounder: "30", colourLabel: "EG" }))
      .toBe("STI-15D30EG");
  });

  it("builds flush sounder relay fire red — STI-15030FR", () => {
    expect(buildEUSModelCode({ mounting: "0", sounder: "30", colourLabel: "FR" }))
      .toBe("STI-15030FR");
  });
...
describe("isValidEUSCombination", () => {
  ...
  it("returns valid for incomplete selection", () => {
    ...
    expect(isValidEUSCombination({ mounting: "0", sounder: "10" })).toEqual({ valid: true });
  });

  it("rejects CK colour — not in allowlist", () => {
    const result = isValidEUSCombination({ mounting: "0", sounder: "10", colourLabel: "CK" });
  ...
  it("rejects NK with surface mount — only valid with flush", () => {
    const resultC = isValidEUSCombination({ mounting: "C", sounder: "10", colourLabel: "NK" });
    const resultD = isValidEUSCombination({ mounting: "D", sounder: "10", colourLabel: "NK" });
    expect(resultC.valid).toBe(false);
    expect(resultD.valid).toBe(false);
  });

  it("accepts NK with flush mount and sounder=10", () => {
    expect(isValidEUSCombination({ mounting: "0", sounder: "10", colourLabel: "NK" }))
      .toEqual({ valid: true });
  });

  it("rejects EG with sounder=10 or sounder=20", () => {
    expect(isValidEUSCombination({ mounting: "0", sounder: "10", colourLabel: "EG" }).valid)
      .toBe(false);
    expect(isValidEUSCombination({ mounting: "D", sounder: "20", colourLabel: "EG" }).valid)
      .toBe(false);
  });

  it("accepts EG with sounder=30 for valid mountings", () => {
    expect(isValidEUSCombination({ mounting: "0", sounder: "30", colourLabel: "EG" }))
      .toEqual({ valid: true });
    expect(isValidEUSCombination({ mounting: "D", sounder: "30", colourLabel: "EG" }))
      .toEqual({ valid: true });
  });

  it("rejects CR colour with flush or 32mm mounting", () => {
    expect(isValidEUSCombination({ mounting: "0", sounder: "10", colourLabel: "CR" }).valid)
      .toBe(false);
    expect(isValidEUSCombination({ mounting: "C", sounder: "10", colourLabel: "CR" }).valid)
      .toBe(false);
  });

  it("accepts CR only with 50mm mounting", () => {
    expect(isValidEUSCombination({ mounting: "D", sounder: "10", colourLabel: "CR" }))
      .toEqual({ valid: true });
  });
});
...
describe("isConfigurationComplete — euroStopper", () => {
  ...
  it("returns false when any step missing", () => {
    expect(isConfigurationComplete(euroStopperModel, {
      mounting: "0", sounder: "10", colourLabel: null,
    })).toBe(false);
  });
  ...
  it("getCompletionPercentage returns correct percentages", () => {
    expect(getCompletionPercentage(euroStopperModel, {
      mounting: null, sounder: null, colourLabel: null,
    })).toBe(0);

    expect(getCompletionPercentage(euroStopperModel, {
      mounting: "0", sounder: null, colourLabel: null,
    })).toBe(33);
    ...
    expect(getCompletionPercentage(euroStopperModel, {
      mounting: "0", sounder: "10", colourLabel: "CB",
    })).toBe(100);
  });
});
